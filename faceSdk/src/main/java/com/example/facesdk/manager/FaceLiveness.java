...
import com.example.facesdk.callback.IFaceDetectCallBack;
import com.example.facesdk.callback.IFaceRegistCalllBack;
import com.example.facesdk.callback.ILivenessCallBack;
import com.example.facesdk.model.LivenessModel;
import com.example.facesdk.utils.FileUtils;
import com.example.facesdk.utils.ImageUtils;
...
import static com.example.facesdk.manager.FaceLiveness.TaskType.TASK_TYPE_ONETON;
...
public class FaceLiveness {
    ...
    private UserInfoManager.UserInfoListener mUserInfoListener = new UserListener();
    ...
    public void registFace(LivenessModel livenessModel) {
        long sTime = System.currentTimeMillis();
        UserInfoManager.getInstance().getFeatureInfo(registNickName, mUserInfoListener);
        if ((GlobalSet.getLiveStatusValue() == GlobalSet.LIVE_STATUS.NO)
                || (GlobalSet.getLiveStatusValue() == GlobalSet.LIVE_STATUS.RGB
                && livenessModel.getRgbLivenessScore() > GlobalSet.getLiveRgbValue())
                || (GlobalSet.getLiveStatusValue() == GlobalSet.LIVE_STATUS.RGN_NIR
            ...
            float length = FaceSDKManager.getInstance().getFaceFeature().extractFeature(
                    ...
                    bytes,
                    livenessModel.getLandmarks());
            if (length == 128 && listFeature.size()<1) {
